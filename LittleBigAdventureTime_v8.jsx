...
import {
  ...
  ShoppingBag, Star, Tag, MapPin, Calendar, Phone, Eye, EyeOff, LogIn, LogOut, Edit3, Camera
} from "lucide-react";

// ─── ENV HELPER ──────────────────────────────────────────────────────────────
// In Vercel: vite replaces these strings at build time via vite.config.js define
// In artifact preview: returns undefined gracefully
const ENV_OS_KEY = typeof __VITE_OS_KEY__ !== 'undefined' ? __VITE_OS_KEY__ : undefined;
const ENV_AI_KEY = typeof __VITE_AI_KEY__ !== 'undefined' ? __VITE_AI_KEY__ : undefined;
const getEnv = (key) => {
  if (key === 'VITE_OS_API_KEY') return ENV_OS_KEY;
  if (key === 'VITE_ANTHROPIC_API_KEY') return ENV_AI_KEY;
  return undefined;
};
...
.auth-card{animation:slide-up .4s cubic-bezier(.34,1.1,.64,1) both}
.leaflet-container{background:#0c2340!important}
.leaflet-control-zoom{border:none!important;box-shadow:none!important}
.leaflet-control-zoom a{background:rgba(10,18,30,0.95)!important;border:1px solid rgba(255,255,255,0.15)!important;color:#e5e7eb!important;border-radius:8px!important;margin-bottom:4px!important}
.leaflet-control-zoom a:hover{background:rgba(34,197,94,0.2)!important;color:#4ade80!important}
.lbat-marker{display:flex;align-items:center;justify-content:center;border-radius:50%;border:2.5px solid #4a6741;background:#1e2d3d;font-size:18px;width:44px;height:44px;cursor:pointer;box-shadow:0 2px 8px rgba(0,0,0,0.5)}
.lbat-marker.seasonal{border-color:#fbbf24;animation:pulse-gold 2.5s ease-in-out infinite}
.lbat-marker.folklore{border-color:#ef4444;background:#1a0808;animation:pulse-red 2s ease-in-out infinite}
.lbat-marker.highlighted{border-color:#22c55e;border-width:3px;box-shadow:0 0 0 4px rgba(34,197,94,0.3),0 2px 8px rgba(0,0,0,0.5)}
`;
...
// ─── AUTH SCREEN ──────────────────────────────────────────────────────────────
const AVATARS = ["🏊","🧗","🏄","🚵","🏕️","🦅","🐺","🌊","🏔️","🦌","🌿","⚡"];
const INTERESTS = ["Wild Swimming","Hiking","Surfing","Wildlife","Kayaking","Coasteering","Photography","Foraging","History","Folklore"];

function AuthScreen({ onAuth }) {
  const [mode, setMode] = useState("welcome"); // welcome | login | signup | setup
  const [form, setForm] = useState({ name:"", email:"", password:"", location:"", avatar:"🏊", interests:[] });
  ...
  const handleLogin = async () => {
    ...
    const saved = localStorage.getItem("lbat:user");
    if (saved) {
      const user = JSON.parse(saved);
      if (user.email === form.email) { onAuth(user); return; }
    }
    setErr("No account found. Try signing up."); setLoading(false);
  };

  const handleSignup = () => {
    if (!form.name || !form.email || !form.password) { setErr("Please fill in all fields"); return; }
    if (form.password.length < 6) { setErr("Password must be at least 6 characters"); return; }
    setErr(""); setMode("setup");
  };

  const handleSetup = () => {
    const user = { ...form, joined: new Date().toISOString(), level:1, advPts:0, guardianPts:0, regionsExplored:[], questsCompleted:0 };
    localStorage.setItem("lbat:user", JSON.stringify(user));
    onAuth(user);
  };

  const inputStyle = { width:"100%", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:14, padding:"14px 16px", color:"#e5e7eb", fontSize:15, marginBottom:12 };

  if (mode === "welcome") return (
    <div style={{height:"100vh",background:"#0d1117",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,textAlign:"center"}}>
      <div style={{fontSize:72,marginBottom:8,animation:"float 3s ease-in-out infinite"}}>🏴󠁧󠁢󠁷󠁬󠁳󠁥</div>
      <h1 style={{fontFamily:"'Fredoka',sans-serif",fontSize:34,color:"#fff",marginBottom:6,lineHeight:1.1}}>Little Big<br/>Adventure Time</h1>
      <p style={{fontSize:14,color:"#6b7280",marginBottom:8,lineHeight:1.6}}>Discover Wales's wildest spots, folklore, and adventures — curated by locals.</p>
      <div style={{display:"flex",gap:8,marginBottom:32,flexWrap:"wrap",justifyContent:"center"}}>
        {["🐬 Cardigan Bay","🐧 Pembrokeshire","🏔️ Snowdonia","🐉 Gower","⛰️ Brecon"].map(r=>(
          <span key={r} style={{fontSize:11,color:"#22c55e",background:"rgba(34,197,94,0.1)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:20,padding:"3px 10px"}}>{r}</span>
        ))}
      </div>
      <div style={{width:"100%",maxWidth:340,display:"flex",flexDirection:"column",gap:10}}>
        <button onClick={()=>setMode("signup")} style={{width:"100%",padding:"16px",background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",borderRadius:18,cursor:"pointer",fontFamily:"'Fredoka',sans-serif",fontSize:18,fontWeight:700,color:"#fff",boxShadow:"0 4px 20px rgba(34,197,94,0.35)"}}>
          Start Your Adventure →
        </button>
        <button onClick={()=>setMode("login")} style={{width:"100%",padding:"14px",background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:18,cursor:"pointer",fontFamily:"'Fredoka',sans-serif",fontSize:16,color:"#9ca3af"}}>
          I already have an account
        </button>
      </div>
      <p style={{fontSize:11,color:"#374151",marginTop:20}}>Free · No ads · Built for adventurers</p>
    </div>
  );

  if (mode === "login") return (
    <div style={{height:"100vh",background:"#0d1117",display:"flex",flexDirection:"column",padding:24,justifyContent:"center"}}>
      <button onClick={()=>setMode("welcome")} style={{background:"none",border:"none",color:"#6b7280",fontSize:13,cursor:"pointer",textAlign:"left",marginBottom:24,padding:0}}>← Back</button>
      <h2 style={{fontFamily:"'Fredoka',sans-serif",fontSize:28,color:"#fff",marginBottom:4}}>Welcome back</h2>
      <p style={{fontSize:14,color:"#6b7280",marginBottom:28}}>Sign in to your adventure account</p>
      <input style={inputStyle} type="email" placeholder="Email address" value={form.email} onChange={e=>upd("email",e.target.value)}/>
      <div style={{position:"relative",marginBottom:12}}>
        <input style={{...inputStyle,marginBottom:0,paddingRight:48}} type={showPw?"text":"password"} placeholder="Password" value={form.password} onChange={e=>upd("password",e.target.value)}/>
        <button onClick={()=>setShowPw(!showPw)} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#6b7280"}}>{showPw?<EyeOff size={16}/>:<Eye size={16}/>}</button>
      </div>
      {err&&<p style={{color:"#f87171",fontSize:13,marginBottom:12}}>{err}</p>}
      <button onClick={handleLogin} disabled={loading} style={{width:"100%",padding:"16px",background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",borderRadius:18,cursor:"pointer",fontFamily:"'Fredoka',sans-serif",fontSize:17,fontWeight:700,color:"#fff",opacity:loading?0.7:1}}>
        {loading?"Signing in…":"Sign In"}
      </button>
      <button onClick={()=>setMode("signup")} style={{marginTop:16,background:"none",border:"none",color:"#6b7280",fontSize:13,cursor:"pointer"}}>No account? Sign up →</button>
    </div>
  );

  if (mode === "signup") return (
    <div style={{height:"100vh",background:"#0d1117",display:"flex",flexDirection:"column",padding:24,justifyContent:"center",overflowY:"auto"}}>
      <button onClick={()=>setMode("welcome")} style={{background:"none",border:"none",color:"#6b7280",fontSize:13,cursor:"pointer",textAlign:"left",marginBottom:24,padding:0}}>← Back</button>
      <h2 style={{fontFamily:"'Fredoka',sans-serif",fontSize:28,color:"#fff",marginBottom:4}}>Create account</h2>
      <p style={{fontSize:14,color:"#6b7280",marginBottom:28}}>Join thousands of Welsh adventurers</p>
      <input style={inputStyle} type="text" placeholder="Your name" value={form.name} onChange={e=>upd("name",e.target.value)}/>
      <input style={inputStyle} type="email" placeholder="Email address" value={form.email} onChange={e=>upd("email",e.target.value)}/>
      <div style={{position:"relative",marginBottom:12}}>
        <input style={{...inputStyle,marginBottom:0,paddingRight:48}} type={showPw?"text":"password"} placeholder="Password (min 6 chars)" value={form.password} onChange={e=>upd("password",e.target.value)}/>
        <button onClick={()=>setShowPw(!showPw)} style={{position:"absolute",right:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:"#6b7280"}}>{showPw?<EyeOff size={16}/>:<Eye size={16}/>}</button>
      </div>
      {err&&<p style={{color:"#f87171",fontSize:13,marginBottom:12}}>{err}</p>}
      <button onClick={handleSignup} style={{width:"100%",padding:"16px",background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",borderRadius:18,cursor:"pointer",fontFamily:"'Fredoka',sans-serif",fontSize:17,fontWeight:700,color:"#fff"}}>
        Continue →
      </button>
    </div>
  );
  ...
  if (mode === "setup") return (
    <div style={{height:"100vh",background:"#0d1117",overflowY:"auto",padding:"24px 24px 40px"}}>
      <div style={{paddingTop:20}}>
        <h2 style={{fontFamily:"'Fredoka',sans-serif",fontSize:26,color:"#fff",marginBottom:4}}>Set up your profile</h2>
        <p style={{fontSize:13,color:"#6b7280",marginBottom:24}}>Tell us about yourself so we can find your perfect adventures</p>

        {/* Avatar picker */}
        <div style={{marginBottom:20}}>
          <div style={{fontFamily:"'Fredoka',sans-serif",fontSize:14,color:"#e5e7eb",marginBottom:10}}>Choose your explorer avatar</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:8}}>
            {AVATARS.map(a=>(
              <button key={a} onClick={()=>upd("avatar",a)} style={{aspectRatio:"1",borderRadius:14,background:form.avatar===a?"rgba(34,197,94,0.15)":"rgba(255,255,255,0.04)",border:form.avatar===a?"2px solid #22c55e":"1px solid rgba(255,255,255,0.08)",cursor:"pointer",fontSize:24,display:"flex",alignItems:"center",justifyContent:"center"}}>{a}</button>
            ))}
          </div>
        </div>
        ...
        {/* Location */}
        <div style={{marginBottom:20}}>
          <div style={{fontFamily:"'Fredoka',sans-serif",fontSize:14,color:"#e5e7eb",marginBottom:10}}>Where are you based?</div>
          <input style={inputStyle} type="text" placeholder="e.g. Cardiff, Aberystwyth, Bristol…" value={form.location} onChange={e=>upd("location",e.target.value)}/>
        </div>

        {/* Interests */}
        <div style={{marginBottom:28}}>
          <div style={{fontFamily:"'Fredoka',sans-serif",fontSize:14,color:"#e5e7eb",marginBottom:10}}>What do you love? (pick any)</div>
          <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
            {INTERESTS.map(i=>{
              const on = form.interests.includes(i);
              return <button key={i} onClick={()=>upd("interests",on?form.interests.filter(x=>x!==i):[...form.interests,i])} style={{padding:"7px 13px",borderRadius:14,background:on?"rgba(34,197,94,0.15)":"rgba(255,255,255,0.04)",color:on?"#4ade80":"#9ca3af",border:on?"1px solid rgba(34,197,94,0.4)":"1px solid rgba(255,255,255,0.08)",fontSize:12,cursor:"pointer",fontWeight:on?600:400}}>{i}</button>;
            })}
          </div>
        </div>

        <button onClick={handleSetup} style={{width:"100%",padding:"16px",background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",borderRadius:18,cursor:"pointer",fontFamily:"'Fredoka',sans-serif",fontSize:18,fontWeight:700,color:"#fff",boxShadow:"0 4px 20px rgba(34,197,94,0.3)"}}>
          Let's Go! 🏴󠁧󠁢󠁷󠁬󠁳󠁥
        </button>
      </div>
    </div>
...
function AskGuide({ region, onClose, onShowSpots }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      // Extract spot mentions from the reply
      const mentioned = allSpots.filter(s => reply.toLowerCase().includes(s.name.toLowerCase()));
      if (mentioned.length > 0) setSpotSuggestions(mentioned.slice(0,4));
      else setSpotSuggestions([]);
      setMsgs(prev=>[...prev,{role:"assistant",text:reply,spots:mentioned.slice(0,4)}]);
    } catch(e) {
      ...
      <div style={{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",touchAction:"pan-y",padding:"16px 16px 0"}}>
        {msgs.map((m,i)=>(
          <div key={i} style={{marginBottom:14,display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
            {m.role==="assistant"&&<div style={{width:28,height:28,borderRadius:"50%",background:"#1e3a2f",border:"1px solid #22c55e",display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,flexShrink:0,marginRight:8,marginTop:2}}>🏴󠁧󠁢󠁷󠁬󠁳󠁥</div>}
            <div style={{maxWidth:"78%"}}>
              <div style={{background:m.role==="user"?"rgba(34,197,94,0.15)":"#1f2937",border:m.role==="user"?"1px solid rgba(34,197,94,0.25)":"1px solid rgba(255,255,255,0.06)",borderRadius:m.role==="user"?"18px 18px 4px 18px":"18px 18px 18px 4px",padding:"10px 14px"}}>
                <p style={{fontFamily:"'Crimson Pro',serif",fontSize:15,lineHeight:1.7,color:m.role==="user"?"#4ade80":"#d1d5db",margin:0}}>{m.text}</p>
              </div>
              {/* Spot chips — show on map */}
              {m.spots && m.spots.length > 0 && (
                <div style={{marginTop:8}}>
                  <div style={{fontSize:10,color:"#6b7280",marginBottom:4}}>📍 Mentioned spots — tap to find on map:</div>
                  <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                    {m.spots.map(s=>(
                      <button key={s.id} onClick={()=>onShowSpots(s)} style={{display:"flex",alignItems:"center",gap:5,background:"rgba(34,197,94,0.1)",border:"1px solid rgba(34,197,94,0.3)",borderRadius:20,padding:"5px 10px",cursor:"pointer",color:"#4ade80",fontSize:11,fontWeight:600}}>
                        <span>{s.emoji}</span>{s.name}<MapPin size={9}/>
                      </button>
                    ))}
                  </div>
                </div>
...
// ─── OS MAP PANEL (Leaflet + OS Data Hub) ────────────────────────────────────
function MapPanel({ region, activeFilter, onSpot, onFolklore, onRoute, highlightSpot }) {
  ...
  function initMap() {
    ...
    const map = L.map(mapRef.current, {
      center: [r.userLoc.lat, r.userLoc.lng],
      zoom: 11,
      zoomControl: true,
    ...
    // OS Data Hub tile layer
    const osKey = getEnv('VITE_OS_API_KEY');
    if (osKey) {
      L.tileLayer(
        `https://api.os.uk/maps/raster/v1/zxy/Outdoor_3857/{z}/{x}/{y}.png?key=${osKey}`,
        { attribution: "© Crown copyright and database rights 2024 OS", maxZoom: 20, minZoom: 7 }
      ).addTo(map);
    } else {
      ...
      navigator.geolocation.getCurrentPosition(
        pos => {
          ...
          const userIcon = L.divIcon({
            className: "",
            html: `<div style="width:20px;height:20px;border-radius:50%;background:#3b82f6;border:3px solid #fff;box-shadow:0 0 0 6px rgba(59,130,246,0.3)"></div>`,
  ...
  function getRegionBounds(reg) {
    const lats = reg.spots.map(s=>s.lat);
    const lngs = reg.spots.map(s=>s.lng);
    return [[Math.min(...lats)-0.05, Math.min(...lngs)-0.05],[Math.max(...lats)+0.05, Math.max(...lngs)+0.05]];
  }

  function addMarkers(map, reg, filter) {
    const L = window.L;
    // Clear old markers
    markersRef.current.forEach(m => m.remove());
    markersRef.current = [];

    const spots = filter==="All"||filter==="📜 Folklore" ? reg.spots : reg.spots.filter(s=>s.activity===filter||s.tags?.includes(filter));
    const showFolk = filter==="All"||filter==="📜 Folklore";

    // Spot markers
    spots.forEach(s => {
      const icon = L.divIcon({
        className: "",
        html: `<div class="lbat-marker${s.seasonal?" seasonal":""}"><span style="font-size:18px">${s.emoji}</span></div>`,
    ...
    // Folklore markers
    if (showFolk) {
      reg.folklore.forEach(f => {
        const icon = L.divIcon({
          className: "",
          html: `<div class="lbat-marker folklore"><span style="font-size:18px">${f.emoji}</span></div>`,
          iconSize: [44,44], iconAnchor: [22,22]
        });
        const m = L.marker([f.lat, f.lng], { icon }).addTo(map);
        m.on("click", () => onFolklore(f));
        markersRef.current.push(m);
      });
    }

    // Route polylines
    reg.routes.forEach((route, ri) => {
      const colors = ["#3b82f6","#8b5cf6","#06b6d4"];
      const col = colors[ri % colors.length];
      const line = L.polyline(route.wp.map(wp=>[wp[0],wp[1]]), {
        color: col, weight: 4, opacity: 0.8, dashArray: "10,6"
      }).addTo(map);
      line.on("click", () => onRoute(route));
  ...
  useEffect(() => {
    if (leafletRef.current && window.L) {
      addMarkers(leafletRef.current, r, activeFilter);
    }
  }, [activeFilter]);

  // Highlight a specific spot (from AI Guide)
  useEffect(() => {
    if (!highlightSpot || !leafletRef.current || !window.L) return;
    const L = window.L;
    leafletRef.current.flyTo([highlightSpot.lat, highlightSpot.lng], 13, { animate: true, duration: 1.2 });
    // Flash the marker
    markersRef.current.forEach(m => {
      if (m.getLatLng && m.getLatLng().lat === highlightSpot.lat) {
        m.openPopup?.();
      }
    });
    ...
    const hIcon = L.divIcon({
      className: "",
      html: `<div class="lbat-marker highlighted"><span style="font-size:18px">${highlightSpot.emoji}</span></div>`,
      iconSize: [44,44], iconAnchor: [22,22]
    });
    const hm = L.marker([highlightSpot.lat, highlightSpot.lng], { icon: hIcon })
      .addTo(leafletRef.current)
      .bindPopup(`<b>${highlightSpot.name}</b><br/>${highlightSpot.activity}`)
      .openPopup();
    markersRef.current.push(hm);
    setTimeout(() => hm.closePopup(), 4000);
  }, [highlightSpot]);

  // Centre on user
  const goToUser = () => {
    if (userPos && leafletRef.current) {
      leafletRef.current.flyTo([userPos.lat, userPos.lng], 13);
    }
  };
  ...
  return (
    <div style={{position:"absolute",inset:0,overflow:"hidden"}}>
      ...
      <button onClick={goToUser} style={{position:"absolute",bottom:90,right:12,zIndex:500,width:40,height:40,borderRadius:10,background:"rgba(10,18,30,0.95)",border:`1px solid ${userPos?"rgba(59,130,246,0.5)":"rgba(255,255,255,0.15)"}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}} title="Go to my location">
        <Navigation size={16} color={userPos?"#60a5fa":"#6b7280"}/>
      </button>
      {locError&&<div style={{position:"absolute",bottom:140,right:12,zIndex:500,background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.3)",borderRadius:10,padding:"6px 10px",fontSize:10,color:"#f87171"}}>📍 Location unavailable</div>}
      {!getEnv('VITE_OS_API_KEY')&&<div style={{position:"absolute",bottom:50,left:"50%",transform:"translateX(-50%)",zIndex:500,background:"rgba(245,158,11,0.1)",border:"1px solid rgba(245,158,11,0.3)",borderRadius:10,padding:"5px 12px",fontSize:10,color:"#fbbf24",whiteSpace:"nowrap"}}>⚠️ Add VITE_OS_API_KEY for OS maps</div>}
    </div>
  );
}
// ─── SPOT SHEET ───────────────────────────────────────────────────────────────
...
function ProfileTab({ user, onSignOut }) {
  ...
  const advPts = questCount * 75 + spotCount * 15 + (user?.advPts||0);
  const guardianPts = Math.floor(advPts * 0.12);
  const levelThresholds = [0,500,1200,2500,4500,7500,12000];
  const level = levelThresholds.findIndex(t=>advPts<t) - 1;
  const levelNames = ["Wanderer","Explorer","Trail Scout","Bay Wanderer","Peak Seeker","Ridge Runner","Guardian"];
  const levelName = levelNames[Math.max(0,level)] || "Guardian";
  const thisLvlPts = levelThresholds[Math.max(0,level)] || 0;
  const nextLvlPts = levelThresholds[Math.min(levelNames.length-1, level+1)] || levelThresholds[levelThresholds.length-1];
  const pct = Math.round(((advPts - thisLvlPts) / (nextLvlPts - thisLvlPts)) * 100);

  const badges = [
    {e:"🏊",n:"Wild Swimmer",on:true,cat:"adventure"},{e:"🐬",n:"Dolphin Spotter",on:true,cat:"adventure"},
    {e:"🌊",n:"Tidal Expert",on:questCount>5,cat:"adventure"},{e:"🌿",n:"Earth Friend",on:true,cat:"stewardship"},
    {e:"📖",n:"Storyteller",on:spotCount>0,cat:"story"},{e:"📜",n:"Lorekeeper",on:false,cat:"story"},
    {e:"🦭",n:"Seal Keeper",on:false,cat:"stewardship"},{e:"🏄",n:"Surf Soul",on:false,cat:"adventure"},
    {e:"⚔️",n:"Quest Master",on:questCount>10,cat:"quest"},{e:"🟣",n:"Legendary",on:false,cat:"quest"},
  ...
  return (
    <div style={{overflowY:"auto",WebkitOverflowScrolling:"touch",touchAction:"pan-y",height:"100%",paddingBottom:80}}>
      {/* Header */}
      <div style={{background:"linear-gradient(135deg,#1e3a2f,#0f2218)",padding:"52px 20px 16px"}}>
        <div style={{display:"flex",gap:14,alignItems:"center",marginBottom:16}}>
          <div style={{width:66,height:66,borderRadius:"50%",background:"rgba(0,0,0,0.3)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:36,border:"3px solid #22c55e"}}>
            {user?.avatar||"🏊"}
          </div>
          <div style={{flex:1}}>
            <h2 style={{fontFamily:"'Fredoka',sans-serif",fontSize:22,color:"#fff"}}>{user?.name||"Explorer"}</h2>
            <div style={{fontSize:12,color:"#f59e0b",fontWeight:700}}>{levelName} · Level {Math.max(1,level+1)}</div>
            <div style={{fontSize:12,color:"rgba(255,255,255,0.4)",marginTop:2}}>{user?.location||"Wales 🏴󠁧󠁢󠁷󠁬󠁳󠁥"}</div>
          </div>
          <button onClick={onSignOut} style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,padding:"8px 12px",cursor:"pointer",color:"#6b7280",fontSize:11,display:"flex",alignItems:"center",gap:4}}>
            <LogOut size={12}/>Sign out
          </button>
        </div>
        {/* Points */}
        <div style={{display:"flex",gap:10,marginBottom:14}}>
          <div style={{flex:1,background:"rgba(245,158,11,0.15)",border:"1px solid rgba(245,158,11,0.25)",borderRadius:14,padding:"12px",textAlign:"center"}}>
            <div style={{fontFamily:"'Fredoka',sans-serif",fontSize:22,color:"#fbbf24",fontWeight:700}}>⚔️ {advPts.toLocaleString()}</div>
            <div style={{fontSize:10,color:"rgba(255,255,255,0.4)"}}>Adventure pts</div>
          </div>
          <div style={{flex:1,background:"rgba(34,197,94,0.12)",border:"1px solid rgba(34,197,94,0.25)",borderRadius:14,padding:"12px",textAlign:"center"}}>
            <div style={{fontFamily:"'Fredoka',sans-serif",fontSize:22,color:"#4ade80",fontWeight:700}}>🌿 {guardianPts}</div>
            <div style={{fontSize:10,color:"rgba(255,255,255,0.4)"}}>Guardian pts</div>
          </div>
        </div>
        {/* Level progress */}
        <div style={{background:"rgba(0,0,0,0.3)",borderRadius:12,padding:"12px"}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
            <span style={{fontSize:12,color:"rgba(255,255,255,0.6)",fontWeight:600}}>Level {Math.max(1,level+1)} — {levelName}</span>
            <span style={{fontSize:11,color:"rgba(255,255,255,0.3)"}}>{pct}% to {levelNames[level+2]||"Max"}</span>
          </div>
          <div style={{background:"rgba(255,255,255,0.1)",borderRadius:8,height:6}}>
            <div style={{width:`${Math.max(5,pct)}%`,height:"100%",background:"linear-gradient(90deg,#f59e0b,#22c55e)",borderRadius:8,transition:"width 1s"}}/>
          </div>
        </div>
        {/* Interests */}
        {user?.interests?.length > 0 && (
          <div style={{marginTop:12,display:"flex",flexWrap:"wrap",gap:6}}>
            {user.interests.map(i=><span key={i} style={{fontSize:11,color:"#4ade80",background:"rgba(34,197,94,0.1)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:20,padding:"3px 9px"}}>{i}</span>)}
          </div>
        )}
      </div>

      {/* Tab bar */}
      <div style={{display:"flex",padding:"12px 16px",gap:8,background:"#111827",borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
        {["stats","badges","friends","crews"].map(v=>(
          <button key={v} onClick={()=>setView(v)} style={{flex:1,background:view===v?"rgba(34,197,94,0.12)":"transparent",border:view===v?"1px solid rgba(34,197,94,0.3)":"1px solid transparent",borderRadius:10,padding:"7px 0",cursor:"pointer",color:view===v?"#4ade80":"#6b7280",fontSize:11,fontWeight:600,textTransform:"capitalize"}}>{v}</button>
        ))}
      </div>
      ...
      <div style={{padding:"16px"}}>
        {view==="stats"&&<>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:18}}>
            {[["Spots Saved",spotCount||0],["Quests Done",questCount||0],["Stories Told",0],["Regions Explored",user?.regionsExplored?.length||0]].map(([l,v])=>(
              <div key={l} style={{background:"#1f2937",borderRadius:13,padding:"13px",textAlign:"center"}}>
                <div style={{fontFamily:"'Fredoka',sans-serif",fontSize:22,color:"#e5e7eb",fontWeight:700}}>{v}</div>
                <div style={{fontSize:11,color:"#6b7280"}}>{l}</div>
              </div>
            ))}
          </div>
          <div style={{background:"#1f2937",borderRadius:14,padding:"14px"}}>
            <div style={{fontFamily:"'Fredoka',sans-serif",fontSize:14,color:"#e5e7eb",marginBottom:10}}>🏆 Quest Breakdown</div>
            {[["common",Math.floor(questCount*0.5)],["rare",Math.floor(questCount*0.3)],["epic",Math.floor(questCount*0.15)],["stewardship",Math.floor(questCount*0.05)]].map(([r,n])=>{
              const c=RARITY[r];
              return <div key={r} style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                <RarityBadge rarity={r} sm/>
                <div style={{flex:1,background:"rgba(255,255,255,0.04)",borderRadius:6,height:6}}>
                  <div style={{width:`${Math.max(2, (n/Math.max(1,questCount))*100)}%`,height:"100%",background:c.color,borderRadius:6}}/>
                </div>
                <span style={{fontSize:12,color:c.text,fontWeight:700,width:20,textAlign:"right"}}>{n}</span>
              </div>;
            })}
          </div>
        </>}

        {view==="badges"&&<div>{["adventure","quest","stewardship","story"].map(cat=>(
          <div key={cat} style={{marginBottom:18}}>
            <div style={{fontFamily:"'Fredoka',sans-serif",fontSize:14,color:"#e5e7eb",marginBottom:8}}>{cat==="adventure"?"📍":cat==="quest"?"⚔️":cat==="stewardship"?"🌿":"📖"} {cat.charAt(0).toUpperCase()+cat.slice(1)} Badges</div>
            <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8}}>
              {badges.filter(b=>b.cat===cat).map(b=>(
                <div key={b.n} style={{textAlign:"center",opacity:b.on?1:0.35}}>
                  <div style={{aspectRatio:"1",borderRadius:12,background:b.on?"rgba(245,158,11,0.12)":"#1f2937",border:b.on?"1px solid rgba(245,158,11,0.35)":"1px solid rgba(255,255,255,0.05)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,marginBottom:3}}>{b.on?b.e:"❓"}</div>
                  <div style={{fontSize:9,color:b.on?"#9ca3af":"#4b5563",lineHeight:1.2}}>{b.n}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {view==="friends"&&<div>
          <div style={{fontFamily:"'Fredoka',sans-serif",fontSize:14,color:"#e5e7eb",marginBottom:12}}>Your Crew</div>
          {FRIENDS.map(f=>(
            <div key={f.id} style={{background:"#1f2937",border:"1px solid rgba(255,255,255,0.05)",borderRadius:16,padding:"13px",marginBottom:8,display:"flex",alignItems:"center",gap:12}}>
              <div style={{position:"relative"}}><span style={{fontSize:28}}>{f.avatar}</span><div style={{position:"absolute",bottom:0,right:0,width:10,height:10,borderRadius:"50%",background:f.status==="online"?"#22c55e":"#374151",border:"2px solid #1f2937"}}/></div>
              <div style={{flex:1}}><div style={{fontFamily:"'Fredoka',sans-serif",fontSize:15,color:"#e5e7eb"}}>{f.name}</div><div style={{fontSize:11,color:"#6b7280"}}>{f.status==="online"?`📍 ${f.location}`:f.lastSeen}</div></div>
              <div style={{textAlign:"right"}}><div style={{fontSize:12,color:"#fbbf24",fontWeight:700}}>{f.points.toLocaleString()}</div><div style={{fontSize:10,color:"#4ade80"}}>🌿 {f.guardian}</div></div>
            </div>
          ))}
          <button style={{width:"100%",background:"rgba(59,130,246,0.08)",border:"1px dashed rgba(59,130,246,0.3)",borderRadius:14,padding:"13px",cursor:"pointer",color:"#60a5fa",fontFamily:"'Fredoka',sans-serif",fontSize:14,marginTop:4}}>+ Add Friends</button>
        </div>}

        {view==="crews"&&<div>
          <div style={{fontFamily:"'Fredoka',sans-serif",fontSize:14,color:"#e5e7eb",marginBottom:12}}>My Crews</div>
          {CREWS.map(c=>(
            <div key={c.id} style={{background:"#1f2937",border:"1px solid rgba(255,255,255,0.05)",borderRadius:16,padding:"14px",marginBottom:8}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:6}}>
                <span style={{fontSize:26}}>{c.emoji}</span>
                <div style={{flex:1}}><div style={{fontFamily:"'Fredoka',sans-serif",fontSize:15,color:"#e5e7eb"}}>{c.name}</div><div style={{fontSize:12,color:"#6b7280"}}>{c.members} members</div></div>
                <span style={{background:c.private?"rgba(245,158,11,0.12)":"rgba(34,197,94,0.12)",color:c.private?"#fbbf24":"#4ade80",padding:"3px 8px",borderRadius:8,fontSize:10,fontWeight:600}}>{c.private?"🔒 Private":"🌍 Open"}</span>
              </div>
            </div>
          ))}
          <button style={{width:"100%",background:"rgba(245,158,11,0.08)",border:"1px dashed rgba(245,158,11,0.3)",borderRadius:14,padding:"13px",cursor:"pointer",color:"#f59e0b",fontFamily:"'Fredoka',sans-serif",fontSize:14,marginTop:4}}>+ Create a New Crew</button>
        </div>}
      </div>
    </div>
  );
...
export default function App() {
  ...
  const handleSignOut = () => { setUser(null); localStorage.removeItem("lbat:user"); };

  // When Guide suggests a spot → navigate to map and highlight it
  const handleShowSpot = (s) => {
    // Find which region this spot belongs to
    const regionId = Object.entries(REGIONS).find(([,r])=>r.spots.some(rs=>rs.id===s.id))?.[0];
    if (regionId) {
      setRId(regionId);
      setTab("map");
      setGuide(false);
      setHighlightSpot(s);
      setTimeout(()=>setHighlightSpot(null), 5000);
    }
  };
  ...
  if (!authChecked) return <div style={{height:"100vh",background:"#0d1117",display:"flex",alignItems:"center",justifyContent:"center"}}><div style={{fontSize:48}} className="float">🏴󠁧󠁢󠁷󠁬󠁳󠁥</div></div>;

  // Show auth screen if not logged in
  if (!user) return <div style={{maxWidth:430,margin:"0 auto",height:"100vh",overflow:"hidden"}}><AuthScreen onAuth={handleAuth}/></div>;
  ...
  return (
    <div style={{maxWidth:430,margin:"0 auto",height:"100vh",background:"#0d1117",position:"relative",overflow:"hidden",display:"flex",flexDirection:"column",fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{flex:1,position:"relative",overflow:"hidden"}}>
        ...
        {tab==="map"&&(
          <div style={{position:"absolute",inset:0,display:"flex",flexDirection:"column"}}>
            {!region
              ? <>
                  <OverviewMap onRegionSelect={selectRegion}/>
                  <button onClick={()=>setGuide(true)} style={{position:"absolute",bottom:80,right:14,zIndex:250,background:"linear-gradient(135deg,#1e3a2f,#0f2218)",border:"1px solid rgba(34,197,94,0.4)",borderRadius:20,padding:"10px 16px",color:"#4ade80",fontSize:12,fontWeight:700,cursor:"pointer",fontFamily:"'Fredoka',sans-serif",display:"flex",alignItems:"center",gap:6}}>🏴󠁧󠁢󠁷󠁬󠁳󠁥 Ask the Guide</button>
                  {guide&&<AskGuide onClose={()=>setGuide(false)} onShowSpots={handleShowSpot}/>}
                </>
              : <>
                  <MapPanel region={region} activeFilter={filter} onSpot={onSpot} onFolklore={onFolklore} onRoute={onRoute} highlightSpot={highlightSpot}/>
                  {/* Map header overlay */}
                  <div style={{position:"absolute",top:0,left:0,right:0,zIndex:400,pointerEvents:"none"}}>
                    <div style={{padding:"12px 12px 0",display:"flex",gap:8,pointerEvents:"all",alignItems:"center"}}>
                      <button onClick={()=>{ setRId(null); clearAll(); setFilter("All"); }} style={{background:"rgba(10,18,30,0.93)",backdropFilter:"blur(12px)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,padding:"9px 12px",color:"#9ca3af",fontSize:12,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",fontWeight:600,whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:4}}>← Wales</button>
                      ...
                      <button onClick={()=>setCommunity(true)} style={{background:"rgba(10,18,30,0.93)",backdropFilter:"blur(12px)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,padding:"9px 12px",cursor:"pointer",display:"flex",alignItems:"center"}}><Users size={16} color="#6b7280"/></button>
                      <button onClick={()=>setMood(true)} style={{background:"linear-gradient(135deg,#f59e0b,#ef4444)",border:"none",borderRadius:14,padding:"9px 12px",color:"#fff",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"'Fredoka',sans-serif",display:"flex",alignItems:"center",gap:4,whiteSpace:"nowrap"}}><Sparkles size={12}/>Up for?</button>
                      <button onClick={()=>setGuide(true)} style={{background:"linear-gradient(135deg,#1e3a2f,#0f2218)",border:"1px solid rgba(34,197,94,0.4)",borderRadius:14,padding:"9px 12px",color:"#4ade80",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"'Fredoka',sans-serif",display:"flex",alignItems:"center",gap:4,whiteSpace:"nowrap"}}>🏴󠁧󠁢󠁷󠁬󠁳󠁥 Guide</button>
                    </div>
                    {/* Region badge */}
                    <div style={{padding:"6px 12px 0",pointerEvents:"none"}}>
                      <div style={{display:"inline-flex",alignItems:"center",gap:6,background:"rgba(10,18,30,0.85)",backdropFilter:"blur(8px)",borderRadius:20,padding:"4px 12px",border:"1px solid rgba(34,197,94,0.2)"}}>
                        <span style={{fontSize:14}}>{region.emoji}</span>
                        <span style={{fontFamily:"'Fredoka',sans-serif",fontSize:12,color:"#4ade80",fontWeight:700}}>{region.name}</span>
                        <span style={{fontSize:11,color:"#4b5563"}}>·</span>
                        <span style={{fontSize:11,color:"#6b7280"}}>{region.spots.length} spots</span>
                      </div>
                    </div>
                    {/* Filter chips */}
                    <div style={{display:"flex",gap:7,padding:"6px 12px",overflowX:"auto",WebkitOverflowScrolling:"touch",touchAction:"pan-x",pointerEvents:"all",scrollbarWidth:"none"}} onTouchStart={e=>e.stopPropagation()} onTouchMove={e=>e.stopPropagation()}>
                      {FILTERS.map(f=><button key={f} onClick={()=>setFilter(f)} style={{flexShrink:0,padding:"4px 12px",borderRadius:18,background:filter===f?"#22c55e":"rgba(10,18,30,0.9)",color:filter===f?"#fff":"#9ca3af",border:filter===f?"none":"1px solid rgba(255,255,255,0.1)",fontSize:11,fontWeight:600,cursor:"pointer",backdropFilter:"blur(8px)"}}>{f}</button>)}
                    </div>
                  </div>
                  {/* Weather bar */}
                  <div style={{position:"absolute",bottom:0,left:0,right:0,zIndex:400,background:"rgba(8,14,22,0.94)",backdropFilter:"blur(12px)",borderTop:"1px solid rgba(255,255,255,0.05)",padding:"7px 14px",display:"flex",gap:14,overflowX:"auto"}}>
                    <div style={{display:"flex",alignItems:"center",gap:5,flexShrink:0}}><Sun size={12} color="#fbbf24"/><div><div style={{fontSize:12,fontWeight:600,color:"#e5e7eb",lineHeight:1}}>{region.wx.temp}</div><div style={{fontSize:10,color:"#6b7280"}}>Temp</div></div></div>
                    <div style={{display:"flex",alignItems:"center",gap:5,flexShrink:0}}><Wind size={12} color="#60a5fa"/><div><div style={{fontSize:12,fontWeight:600,color:"#e5e7eb",lineHeight:1}}>{region.wx.wind}</div><div style={{fontSize:10,color:"#6b7280"}}>Wind</div></div></div>
                  ...
                  {/* Overlays */}
                  {guide&&<AskGuide region={region} onClose={()=>setGuide(false)} onShowSpots={handleShowSpot}/>}
          ...
          {tab==="explore"&&(
            region
              ...
              : <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",padding:24,textAlign:"center"}}>
                  ...
                  <h2 style={{fontFamily:"'Fredoka',sans-serif",fontSize:22,color:"#fff",marginBottom:8}}>Pick a Region</h2>
                  <p style={{color:"#6b7280",marginBottom:20}}>Head to the Map tab and choose a region to explore.</p>
                  <div style={{display:"flex",gap:10,flexWrap:"wrap",justifyContent:"center"}}>{Object.values(REGIONS).map(r=><button key={r.id} onClick={()=>selectRegion(r.id)} style={{background:"#1f2937",border:"1px solid rgba(34,197,94,0.25)",borderRadius:14,padding:"14px 10px",cursor:"pointer",minWidth:90}}><div style={{fontSize:24,marginBottom:4}}>{r.emoji}</div><div style={{fontFamily:"'Fredoka',sans-serif",fontSize:13,color:"#e5e7eb"}}>{r.name}</div></button>)}</div>
                </div>
          )}
      ...
      <div style={{background:"rgba(8,14,22,0.97)",backdropFilter:"blur(20px)",borderTop:"1px solid rgba(255,255,255,0.05)",display:"flex",padding:"6px 0",zIndex:250,flexShrink:0}}>
        {TABS.map(t=><button key={t.id} onClick={()=>{setTab(t.id);if(t.id!=="map")clearAll();}} style={{flex:1,background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:2,padding:"5px 0",color:tab===t.id?"#22c55e":"#4b5563"}}>
          <div style={{transform:tab===t.id?"scale(1.12)":"scale(1)",transition:"transform .18s"}}>{t.icon}</div>
